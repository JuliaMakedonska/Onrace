...
7. **Gap:** what happens when an already-logged `official_result_url` goes dead by the time a person needs it for an elite-race application? Surfaced while building the Retrieve Proof flow (`../flows.md`, Related Job 4 → `DeadEnd5`), not by this research pass originally — there's no in-app way to edit or re-link a logged result, only to create one (Related Job 3) or retrieve one (Related Job 4). **Hypothesis:** an acceptable MVP gap, not a launch blocker — the self-reported-link trust model already accepts that Onrace doesn't police a link's ongoing validity (see gap 4, above), and no job in `jtbd.md` currently asks to *correct* a result after logging it, so building an edit/re-link capability now would be speculative scope, not sourced. Revisit if a "fix a stale result" job is ever validated with real users, or if link rot proves common in practice. **Follows from:** `../sitemap.md`/`../flows.md` — design surfacing a gap this research pass didn't originally anticipate, rather than the usual direction.
