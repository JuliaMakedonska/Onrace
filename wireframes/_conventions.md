...
- **Fixed phone frame** — every screen is wrapped in a fixed-size container that mimics a real phone screen (390×844, iPhone-sized), with its own internal scroll (the frame's content scrolls inside it; the frame itself never stretches or grows with the browser window, and nothing — a drawer, a nav bar — is ever allowed to render outside its bounds). This reflects Onrace's actual scope (`../CLAUDE.md`: web/responsive, no native app) as a phone-first default, and reads as an actual device viewport rather than a resizable desktop page. Captions/spec notes documenting job/flow traceability (see § 5's spec-note convention) live outside the frame, above or below it — they're wireframe documentation, not product UI, so they don't appear inside the device. The frame sits as one clean, self-contained region; the site nav panel (§ 6) is a second, wholly separate region to its left (a CSS grid with fixed column widths, not a flex layout that could let the two blend) — never nested inside one another.
- **Global nav is a bottom tab bar, not top tabs.** Onrace's real in-app 3-item global nav (`../sitemap.md` → Navigation § 1: Discover / Log Result / My Results) is fixed to the bottom edge of the phone frame — the standard mobile placement for a persistent global nav — not styled as browser-style horizontal tabs at the top. It sits outside the frame's scrollable content (so it doesn't scroll away) but still inside the phone frame's own bounds. No icons yet (§ 7) — just the correct bar shape and bottom position; the current tab is marked the same way as any other current-page indicator in this file (inverted fill).
...
- **Layout**: `<body>` wraps everything in `<div class="layout">` — a CSS grid, `grid-template-columns: 240px 1fr` with an explicit gap, not a flex layout — containing `<nav class="site-nav">` (fixed 240px, `position: sticky`, own scroll) as one column and `<div class="page-content">` (the spec-note caption, the phone frame, the frame caption) as the other. Grid guarantees the two stay separate, bounded regions at any window width — the nav column can never overlap or blend into the phone frame's column. The phone frame itself is untouched by this — the nav sits outside it, alongside it, never inside the device viewport.
