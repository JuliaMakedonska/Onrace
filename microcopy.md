...
**Before / After** (added 2026-09-26): **Before** is the line as transcribed. **After** is the line rewritten per `voice.md`, filled in screen by screen as each is rewritten. **Race browse** is done (all four pages); other screens show a blank After until their pass. "unchanged" means the line was checked against `voice.md` and already complies, or isn't ours to rewrite (catalog data, user selections, user-written values, wireframe annotations).

| # | Screen | Zone | Before | After | Why | Type | States | Source | Flags |
|---:|---|---|---|---|---|---|---|---|---|
| 1 | Race browse | (page) | Onrace — Race browse, empty state (wireframe) | unchanged | Wireframe scaffolding, not product copy (P1): replaced by the real map/page, not rewritten | a11y label | empty | wireframe annotation | P1 |
| 2 | Race browse | large title | Discover | unchanged | Heading rule: the place's name; also the tab and the future back label "‹ Discover" | heading | empty; error; loading (state=loading); base | UI copy |  |
| 3 | Race browse | shared controls | Filters · 3 | unchanged | Label + count of active filters | button | empty | UI copy |  |
| 4 | Race browse | shared controls | Choose view | unchanged | Already follows voice.md (Dictionary terms, Microcopy rule for its type) | a11y label | empty; error; loading (state=loading); base | UI copy |  |
| 5 | Race browse | shared controls | List | unchanged | Already follows voice.md (Dictionary terms, Microcopy rule for its type) | button | empty; error; loading (state=loading); base | UI copy |  |
| 6 | Race browse | shared controls | Map | unchanged | Already follows voice.md (Dictionary terms, Microcopy rule for its type) | button | empty; error; loading (state=loading); base | UI copy |  |
| 7 | Race browse | active filter chips | Active filters | unchanged | Already follows voice.md (Dictionary terms, Microcopy rule for its type) | a11y label | empty; error; loading (state=loading); base | UI copy |  |
| 8 | Race browse | active filter chips | Remove filter: Middle East | unchanged | Already follows voice.md (Dictionary terms, Microcopy rule for its type) | a11y label | empty | UI copy + user selection |  |
| 9 | Race browse | active filter chips | Middle East × | unchanged | Not ours to rewrite: user selection (from option list) | button | empty | user selection (from option list) |  |
| 10 | Race browse | active filter chips | Remove filter: Triathlon | unchanged | Already follows voice.md (Dictionary terms, Microcopy rule for its type) | a11y label | empty | UI copy + user selection |  |
| 11 | Race browse | active filter chips | Triathlon × | unchanged | Not ours to rewrite: user selection (from option list) | button | empty | user selection (from option list) |  |
| 12 | Race browse | active filter chips | Remove filter: 1 Oct – 30 Nov 2026 | unchanged | Already follows voice.md (Dictionary terms, Microcopy rule for its type) | a11y label | empty | UI copy + user selection |  |
| 13 | Race browse | active filter chips | 1 Oct – 30 Nov 2026 × | unchanged | Not ours to rewrite: user selection (from option list) | button | empty | user selection (from option list) |  |
| 14 | Race browse | active filter chips | Clear all | **Clear all filters** | Dictionary A5: one label for one action; reads alone (Button rule) | button | empty; error; loading (state=loading); base | UI copy | A5 |
| 15 | Race browse | results row | 0 races match your filters | unchanged | Plain count | body text | empty | UI copy |  |
| 16 | Race browse | results row | Map or list view | unchanged | Already follows voice.md (Dictionary terms, Microcopy rule for its type) | a11y label | empty; error; loading (state=loading); base | UI copy |  |
| 17 | Race browse | empty state | No races match your current filters | unchanged | Empty rule: says why it's empty; the body and buttons give the next step | heading | empty | UI copy |  |
| 18 | Race browse | empty state | Remove a filter above, or try a wider date range, another region, or more sport types. | unchanged | Empty rule: the action, concretely | state message | empty | UI copy |  |
| 19 | Race browse | empty state | Loosen filters | **Change filters** | Button rule: name the result of the tap. It opens the Filters sheet; it doesn't loosen anything by itself | button | empty | UI copy |  |
| 20 | Race browse | empty state | Clear all filters | unchanged | Already the Dictionary term (A5) | button | empty | UI copy | A5 |
| 21 | Race browse | empty state | [ map placeholder — no pins: 0 races match ] | unchanged | Wireframe scaffolding, not product copy (P1): replaced by the real map/page, not rewritten | state message | empty | wireframe annotation | P1 |
| 22 | Race browse | global nav | Global navigation | unchanged | Already follows voice.md (Dictionary terms, Microcopy rule for its type) | a11y label | empty; error; loading (state=loading); base | UI copy |  |
| 23 | Race browse | global nav | Discover | unchanged | Heading rule: the place's name; also the tab and the future back label "‹ Discover" | tab label | empty; error; loading (state=loading); base | UI copy |  |
| 24 | Race browse | global nav | My Results | unchanged | Already follows voice.md (Dictionary terms, Microcopy rule for its type) | tab label | empty; error; loading (state=loading); base | UI copy | T1 |
| 25 | Race browse | global nav | Profile | unchanged | Already follows voice.md (Dictionary terms, Microcopy rule for its type) | tab label | empty; error; loading (state=loading); base | UI copy |  |
| 26 | Race browse | filters sheet | Filters | unchanged | The sheet's name | heading | empty; error; loading (state=loading); base | UI copy |  |
| 27 | Race browse | filters sheet | Region | unchanged | Already follows voice.md (Dictionary terms, Microcopy rule for its type) | field label | empty; error; loading (state=loading); base | UI copy |  |
| 28 | Race browse | filters sheet | All regions | unchanged | Already follows voice.md (Dictionary terms, Microcopy rule for its type) | option | empty; error; loading (state=loading); base | UI copy |  |
| 29 | Race browse | filters sheet | Europe | unchanged | Not ours to rewrite: reference data (option list) | option | empty; error; loading (state=loading); base | reference data (option list) |  |
| 30 | Race browse | filters sheet | North America | unchanged | Not ours to rewrite: reference data (option list) | option | empty; error; loading (state=loading); base | reference data (option list) |  |
| 31 | Race browse | filters sheet | Middle East | unchanged | Not ours to rewrite: reference data (option list) | option | empty; error; loading (state=loading); base | reference data (option list) |  |
| 32 | Race browse | filters sheet | Asia-Pacific | unchanged | Not ours to rewrite: reference data (option list) | option | empty; error; loading (state=loading); base | reference data (option list) |  |
| 33 | Race browse | filters sheet | Africa | unchanged | Not ours to rewrite: reference data (option list) | option | empty; error; loading (state=loading); base | reference data (option list) |  |
| 34 | Race browse | filters sheet | Date from | **Race date from** | Dictionary C1: "race date"; still reads alone as a label | field label | empty; error; loading (state=loading); base | UI copy |  |
| 35 | Race browse | global nav | 2026-10-01 | unchanged | Not ours to rewrite: user-written | field value (prefilled) | empty | user-written |  |
| 36 | Race browse | filters sheet | Date to | **Race date to** | Dictionary C1 | field label | empty; error; loading (state=loading); base | UI copy |  |
| 37 | Race browse | global nav | 2026-11-30 | unchanged | Not ours to rewrite: user-written | field value (prefilled) | empty | user-written |  |
| 38 | Race browse | filters sheet | Sport type | unchanged | Already follows voice.md (Dictionary terms, Microcopy rule for its type) | field label | empty; error; loading (state=loading); base | UI copy |  |
| 39 | Race browse | global nav | HYROX | unchanged | Not ours to rewrite: catalog data | field label | empty; error; loading (state=loading); base | catalog data |  |
| 40 | Race browse | global nav | DEKA / functional fitness | unchanged | Not ours to rewrite: catalog data | field label | empty; error; loading (state=loading); base | catalog data |  |
| 41 | Race browse | global nav | Marathon | unchanged | Not ours to rewrite: catalog data | field label | empty; error; loading (state=loading); base | catalog data |  |
| 42 | Race browse | global nav | Half marathon | unchanged | Not ours to rewrite: catalog data | field label | empty; error; loading (state=loading); base | catalog data |  |
| 43 | Race browse | global nav | Triathlon | unchanged | Not ours to rewrite: catalog data | field label | empty; error; loading (state=loading); base | catalog data |  |
| 44 | Race browse | global nav | Ultra | unchanged | Not ours to rewrite: catalog data | field label | empty; error; loading (state=loading); base | catalog data |  |
| 45 | Race browse | global nav | OCR | unchanged | Not ours to rewrite: catalog data | field label | empty; error; loading (state=loading); base | catalog data |  |
| 46 | Race browse | filters sheet | Close | **Cancel** | Dictionary A3: Cancel leaves a task without changing anything (approved 2026-09-26) | button | empty; error; loading (state=loading); base | UI copy | A3 |
| 47 | Race browse | filters sheet | Apply filters | unchanged | Button rule: verb + object, reads alone | button | empty; error; loading (state=loading); base | UI copy |  |
| 48 | Race browse | (page) | Onrace — Race browse, error state (wireframe) | unchanged | Wireframe scaffolding, not product copy (P1): replaced by the real map/page, not rewritten | a11y label | error | wireframe annotation | P1 |
| 49 | Race browse | shared controls | Filters | unchanged | The sheet's name | button | error; loading (state=loading); base | UI copy |  |
| 50 | Race browse | results row | Races couldn't be loaded | **Races didn’t load** | Plain past tense, not passive; C3 curly apostrophe; doesn't repeat the heading word for word | body text | error | UI copy |  |
| 51 | Race browse | error state | Couldn't load races | **Couldn’t load races** | C3 curly apostrophe; wording already follows the Error rule (what happened) | heading | error | UI copy |  |
| 52 | Race browse | error state | Check your connection and try again. Your filters will stay as they are. | **The connection may have dropped. Check it, then try again. Your filters stay as they are.** | Error rule: adds the missing "why", as a possibility (principle 1: Onrace can't know it dropped), then what to do next | state message | error | UI copy |  |
| 53 | Race browse | error state | Try again | unchanged | Dictionary term. The Button rule wants an accessible name ("Try loading races again"), but that's an attribute, not text, so it's left for the markup pass | button | error | UI copy |  |
| 54 | Race browse | error state | [ map placeholder — no pins: races failed to load ] | unchanged | Wireframe scaffolding, not product copy (P1): replaced by the real map/page, not rewritten | state message | error | wireframe annotation | P1 |
| 55 | Race browse | global nav | 2026-09-23 | unchanged | Not ours to rewrite: user-written | field value (prefilled) | error; loading (state=loading); base | user-written |  |
| 56 | Race browse | global nav | 2027-09-23 | unchanged | Not ours to rewrite: user-written | field value (prefilled) | error; loading (state=loading); base | user-written |  |
| 57 | Race browse | (page) | Onrace — Race browse, loading state (wireframe) | unchanged | Wireframe scaffolding, not product copy (P1): replaced by the real map/page, not rewritten | a11y label | loading | wireframe annotation | P1 |
| 58 | Race browse | results row | Loading races… | unchanged | Loading rule: names what's loading | body text | loading (state=loading) | UI copy |  |
| 59 | Race browse | results row | Sort: | unchanged | Clear with the new option labels | field label | loading (state=loading); base | UI copy |  |
| 60 | Race browse | results row | Soonest first | unchanged | Pairs with "Latest first" | option | loading (state=loading); base | UI copy |  |
| 61 | Race browse | results row | Farthest first | **Latest first** | On a map-first screen "farthest" reads as distance; the sort is by race date | option | loading (state=loading); base | UI copy |  |
| 62 | Race browse | loading state | Map loading; race pins appear once races have loaded | **Loading races on the map…** | Loading rule: name what's loading, no explanation of the mechanics | a11y label | loading (state=loading) | UI copy |  |
| 63 | Race browse | loading state | [ map placeholder — loading race pins… ] | unchanged | Wireframe scaffolding, not product copy (P1): replaced by the real map/page, not rewritten | body text | loading (state=loading) | wireframe annotation | P1 |
| 64 | Race browse | (page) | Onrace — Race browse (wireframe) | unchanged | Wireframe scaffolding, not product copy (P1): replaced by the real map/page, not rewritten | a11y label | base | wireframe annotation | P1 |
| 65 | Race browse | results row | 7 upcoming races | unchanged | Plain count of what's on screen (Forbidden: counts only as facts) | body text | base | UI copy |  |
| 66 | Race browse | results row | Berlin Marathon | unchanged | Not ours to rewrite: catalog data | heading | base | catalog data |  |
| 67 | Race browse | results row | Marathon | unchanged | Not ours to rewrite: catalog data | body text (in tappable row) | base | catalog data |  |
| 68 | Race browse | results row | 27 Sep 2026 | unchanged | Not ours to rewrite: catalog data | body text (in tappable row) | base | catalog data |  |
| 69 | Race browse | results row | Berlin, Germany | unchanged | Not ours to rewrite: catalog data | body text (in tappable row) | base | catalog data |  |
| 70 | Race browse | results row | HYROX London | unchanged | Not ours to rewrite: catalog data | heading | base | catalog data |  |
| 71 | Race browse | results row | HYROX | unchanged | Not ours to rewrite: catalog data | body text (in tappable row) | base | catalog data |  |
| 72 | Race browse | results row | 6 Dec 2026 | unchanged | Not ours to rewrite: catalog data | body text (in tappable row) | base | catalog data |  |
| 73 | Race browse | results row | London, UK | unchanged | Not ours to rewrite: catalog data | body text (in tappable row) | base | catalog data |  |
| 74 | Race browse | results row | HYROX Dallas | unchanged | Not ours to rewrite: catalog data | heading | base | catalog data |  |
| 75 | Race browse | results row | 17 Jan 2027 | unchanged | Not ours to rewrite: catalog data | body text (in tappable row) | base | catalog data |  |
| 76 | Race browse | results row | Dallas, USA | unchanged | Not ours to rewrite: catalog data | body text (in tappable row) | base | catalog data |  |
| 77 | Race browse | results row | DEKA STRONG Chicago | unchanged | Not ours to rewrite: catalog data | heading | base | catalog data |  |
| 78 | Race browse | results row | DEKA | unchanged | Not ours to rewrite: catalog data | body text (in tappable row) | base | catalog data |  |
| 79 | Race browse | results row | 14 Mar 2027 | unchanged | Not ours to rewrite: catalog data | body text (in tappable row) | base | catalog data |  |
| 80 | Race browse | results row | Chicago, USA | unchanged | Not ours to rewrite: catalog data | body text (in tappable row) | base | catalog data |  |
| 81 | Race browse | results row | Boston Marathon | unchanged | Not ours to rewrite: catalog data | heading | base | catalog data |  |
| 82 | Race browse | results row | 20 Apr 2027 | unchanged | Not ours to rewrite: catalog data | body text (in tappable row) | base | catalog data |  |
| 83 | Race browse | results row | Boston, USA | unchanged | Not ours to rewrite: catalog data | body text (in tappable row) | base | catalog data |  |
| 84 | Race browse | results row | Spartan Race Beast — Killington | unchanged | Not ours to rewrite: catalog data | heading | base | catalog data |  |
| 85 | Race browse | results row | OCR | unchanged | Not ours to rewrite: catalog data | body text (in tappable row) | base | catalog data |  |
| 86 | Race browse | results row | 11 Jul 2027 | unchanged | Not ours to rewrite: catalog data | body text (in tappable row) | base | catalog data |  |
| 87 | Race browse | results row | Killington, USA | unchanged | Not ours to rewrite: catalog data | body text (in tappable row) | base | catalog data |  |
| 88 | Race browse | results row | UTMB — Ultra-Trail du Mont-Blanc | unchanged | Not ours to rewrite: catalog data | heading | base | catalog data |  |
| 89 | Race browse | results row | Ultra | unchanged | Not ours to rewrite: catalog data | body text (in tappable row) | base | catalog data |  |
| 90 | Race browse | results row | 24 Aug 2027 | unchanged | Not ours to rewrite: catalog data | body text (in tappable row) | base | catalog data |  |
| 91 | Race browse | results row | Chamonix, France | unchanged | Not ours to rewrite: catalog data | body text (in tappable row) | base | catalog data |  |
| 92 | Race browse | results row | Map of the filtered races | unchanged | Already follows voice.md (Dictionary terms, Microcopy rule for its type) | a11y label | base | UI copy |  |
| 93 | Race browse | results row | Berlin Marathon, Berlin, 27 Sep 2026 | unchanged | Not ours to rewrite: catalog data | a11y label | base | catalog data |  |
| 94 | Race browse | results row | Berlin | unchanged | Not ours to rewrite: catalog data | button | base | catalog data |  |
| 95 | Race browse | results row | HYROX London, London, 6 Dec 2026 | unchanged | Not ours to rewrite: catalog data | a11y label | base | catalog data |  |
| 96 | Race browse | results row | London | unchanged | Not ours to rewrite: catalog data | button | base | catalog data |  |
| 97 | Race browse | results row | UTMB — Ultra-Trail du Mont-Blanc, Chamonix, 24 Aug 2027 | unchanged | Not ours to rewrite: catalog data | a11y label | base | catalog data |  |
| 98 | Race browse | results row | Chamonix | unchanged | Not ours to rewrite: catalog data | button | base | catalog data |  |
| 99 | Race browse | results row | HYROX Dallas, Dallas, 17 Jan 2027 | unchanged | Not ours to rewrite: catalog data | a11y label | base | catalog data |  |
| 100 | Race browse | results row | Dallas | unchanged | Not ours to rewrite: catalog data | button | base | catalog data |  |
| 101 | Race browse | results row | DEKA STRONG Chicago, Chicago, 14 Mar 2027 | unchanged | Not ours to rewrite: catalog data | a11y label | base | catalog data |  |
| 102 | Race browse | results row | Chicago | unchanged | Not ours to rewrite: catalog data | button | base | catalog data |  |
| 103 | Race browse | results row | Spartan Race Beast — Killington, Killington, 11 Jul 2027 | unchanged | Not ours to rewrite: catalog data | a11y label | base | catalog data |  |
| 104 | Race browse | results row | Killington | unchanged | Not ours to rewrite: catalog data | button | base | catalog data |  |
| 105 | Race browse | results row | Boston Marathon, Boston, 20 Apr 2027 | unchanged | Not ours to rewrite: catalog data | a11y label | base | catalog data |  |
| 106 | Race browse | results row | Boston | unchanged | Not ours to rewrite: catalog data | button | base | catalog data |  |
| 107 | Race browse | results row | [ map placeholder — pin positions are approximate; pan/zoom changes what's visible, not which races match ] | unchanged | Wireframe scaffolding, not product copy (P1): replaced by the real map/page, not rewritten | helper text | base | wireframe annotation | P1 |
| 108 | Race browse | shared controls | Filters · 2 | unchanged | Label + count of active filters | button | base,empty (when filters are on) | UI copy |  |
| 109 | Race browse | results row | 3 races match your filters | unchanged | Plain count | body text | base (filtered) | UI copy |  |
| 110 | Race browse | results row | 1 race matches your filters | unchanged | Plain count | body text | base (filtered) | UI copy |  |
